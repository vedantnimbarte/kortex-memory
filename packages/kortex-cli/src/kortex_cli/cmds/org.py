...
app = typer.Typer(help="Manage organizations.", no_args_is_help=True)


@app.command("list")
def list_orgs(json_output: Annotated[bool, typer.Option("--json")] = False) -> None:
...
def create(
    ...
    plan: Annotated[str, typer.Option()] = "free",
...
def show(public_id: str, json_output: Annotated[bool, typer.Option("--json")] = False) -> None:
    with ApiClient() as client:
        try:
            result = client.get(f"/v1/orgs/{public_id}")
        except CliApiError as e:
            fail(str(e))
            return
    print_obj(result, json_output=json_output)

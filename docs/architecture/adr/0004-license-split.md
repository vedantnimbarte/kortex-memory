...
Two pieces of evidence, both from the market research (§3, T9):
...
Today the whole repository is Apache-2.0: the `LICENSE` file (WU-0.1) and the
...
license.** The list, matching WU-3.1 and nothing beyond it:
...
The recording side shipped Apache-2.0 (WU-3.1, audit) and stays that way: the
...
is actually written — which is gated on a named enterprise prospect (WU-3.1).
...
product's whole wedge (§5, "Self-host quality — Win, big").

## Consequences

- The README can state the guarantee in one line, and the comparison page can
  fill the "self-host" column without an asterisk. That is the point.
...
- `ee/` stays empty and unmentioned until WU-3.1 opens. An `ee/` directory with
  a placeholder README is a promise to charge for something, made before there
